// Client Component - Subjects grid section with grade filter
'use client';
...
const SUBJECTS = [
  { id: 'ict',     name: 'ICT',         tint: 'var(--p-blue)',   ink: 'var(--p-ink-blue)',   emoji: '💻', modules: 12, topics: ['Networks', 'Programming', 'Databases', 'Web tech'],   tutor: 'Mr. Silva',     popular: true,  grades: ['O/L', 'A/L'] },
  { id: 'physics', name: 'Physics',     tint: 'var(--p-peach)',  ink: 'var(--p-ink-peach)',  emoji: '⚛️', modules: 9,  topics: ['Mechanics', 'Waves', 'Electricity', 'Modern'],           tutor: 'Ms. Fernando',  popular: false, grades: ['A/L'] },
  { id: 'chem',    name: 'Chemistry',   tint: 'var(--p-mint)',   ink: 'var(--p-ink-mint)',   emoji: '🧪', modules: 8,  topics: ['Organic', 'Inorganic', 'Physical', 'Analysis'],           tutor: 'Mr. Bandara',   popular: true,  grades: ['A/L'] },
  { id: 'math',    name: 'Mathematics', tint: 'var(--p-lav)',    ink: 'var(--p-ink-lav)',    emoji: '📐', modules: 14, topics: ['Algebra', 'Calculus', 'Stats', 'Vectors'],                tutor: 'Mr. Perera',    popular: false, grades: ['O/L', 'A/L'] },
  { id: 'english', name: 'English',     tint: 'var(--p-pink)',   ink: 'var(--p-ink-pink)',   emoji: '📚', modules: 7,  topics: ['Grammar', 'Essays', 'Reading', 'Listening'],              tutor: 'Ms. Jayasing',  popular: false, grades: ['O/L'] },
];

const GRADE_FILTERS = ['All', 'O/L', 'A/L'];

export function SubjectsSection() {
  const t = useT();
  const [grade, setGrade] = useState('All');

  const visible = grade === 'All' ? SUBJECTS : SUBJECTS.filter(s => s.grades.includes(grade));
    ...
    <section id="subjects" style={{ padding: '84px 0', background: 'var(--p-bg-alt)' }}>
      <div style={{ maxWidth: 1240, margin: '0 auto', padding: '0 28px' }}>
        ...
        <div className="subj-head">
          <div>
            ...
            <h2 style={{ fontFamily: 'var(--font-display,Nunito,sans-serif)', fontWeight: 800, fontSize: 'clamp(26px,3.2vw,44px)', letterSpacing: '-0.015em', lineHeight: 1.1, maxWidth: 680, color: 'var(--p-ink)', margin: 0 }}>
              {t.subjects.heading.split('Fifty modules.').length > 1 ? (
                <>
                  {t.subjects.heading.split('Fifty modules.')[0]}
                  ...
                  {t.subjects.heading.split('Fifty modules.')[1]}
                </>
              ) : t.subjects.heading}
            </h2>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {GRADE_FILTERS.map(g => (
                <button key={g} onClick={() => setGrade(g)} style={{ padding: '9px 16px', borderRadius: 999, border: `1px solid ${grade === g ? 'var(--p-primary)' : 'var(--p-line-2)'}`, background: grade === g ? 'var(--p-primary)' : '#fff', color: grade === g ? '#fff' : 'var(--p-ink-2)', fontFamily: 'var(--font-display,Nunito,sans-serif)', fontWeight: 800, fontSize: 13.5, cursor: 'pointer', transition: 'all .12s' }}>
                  {g}
                </button>
              ))}
            </div>
        ...
        <div className="subj-grid">
          {visible.map(s => (
            <article key={s.id} className="card subj-card">
              ...
              <div style={{ padding: '10px 22px 0', display: 'flex', gap: 6 }}>
                {s.grades.map(g => (
                  <span key={g} style={{ display: 'inline-flex', alignItems: 'center', padding: '3px 10px', borderRadius: 999, fontFamily: 'var(--font-display,Nunito,sans-serif)', fontWeight: 700, fontSize: 10.5, background: 'var(--p-bg-alt)', color: 'var(--p-ink-2)' }}>{g}</span>
                ))}
              </div>
          ...
          {/* CTA tile — only show when All or space allows */}
          {(grade === 'All' || visible.length < 5) && (
      ...
      <style>{`
        ...
        .subj-banner { position: relative; height: 120px; padding: 16px; }
        .subj-badge {
        ...
        .subj-body { padding: 30px 22px 22px; display: flex; flex-direction: column; flex: 1; }
        ...
        .subj-explore-btn {
          display: flex; align-items: center; gap: 8px;
          padding: 11px 18px; border: none; border-radius: 12px; color: #fff;
